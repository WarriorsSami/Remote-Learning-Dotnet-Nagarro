...
namespace iQuest.HotelQueries.Tests
{
    ...
    public class Query11_GetConflictingReservationsTests
    {
        ...
        {
            IDictionary<Reservation, List<Reservation>> actualReservations =
                hotel.GetConflictingReservations();

            foreach (
                KeyValuePair<Reservation, List<Reservation>> keyValuePair in actualReservations
            )
            {
                int actualId = keyValuePair.Key.Id;
                int[] actualIds = keyValuePair.Value.Select(x => x.Id).ToArray();
            ...
            CollectionAssert.AreEquivalent(
                expectedReservationsWithConflicts,
                actualReservationsWithConflicts
            );
